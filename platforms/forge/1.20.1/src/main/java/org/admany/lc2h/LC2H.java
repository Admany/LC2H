...
public class LC2H {
    ...
    public static final Logger LOGGER = LogManager.getLogger(LC2H.class);
    ...
    private volatile Handler quantifiedJulBridge;
    ...
    private void redirectQuantifiedJulLogging() {
        try {
            ...
            quantifiedJulBridge = bridge;
            jul.addHandler(bridge);
            refreshQuantifiedLogging();
    ...
    private void preconfigureQuantifiedLogging() {
        Path configRoot;
        try {
            configRoot = net.minecraftforge.fml.loading.FMLPaths.CONFIGDIR.get();
        } catch (Throwable ignored) {
            configRoot = Path.of("config");
        }
        Path[] candidates = {
        ...
        java.util.regex.Pattern consolePattern = java.util.regex.Pattern.compile(
            "(?m)^\\s*\"logToConsole\"\\s*:\\s*(true|false)\\s*,?\\s*$",
            java.util.regex.Pattern.CASE_INSENSITIVE);
        java.util.regex.Pattern levelPattern = java.util.regex.Pattern.compile(
            "(?m)^\\s*\"logLevel\"\\s*:\\s*\"([^\"]+)\"\\s*,?\\s*$",
            java.util.regex.Pattern.CASE_INSENSITIVE);
        for (Path candidate : candidates) {
            ...
            try {
                ...
                java.util.regex.Matcher console = consolePattern.matcher(text);
                java.util.regex.Matcher level = levelPattern.matcher(text);
                boolean hasConsoleSetting = console.find();
                boolean hasLevelSetting = level.find();
                if (!hasConsoleSetting && !hasLevelSetting) {
                    return;
                }
                boolean enabled = !hasConsoleSetting || Boolean.parseBoolean(console.group(1));
                String configuredLevel = hasLevelSetting ? level.group(1) : "INFO";
                applyQuantifiedLogging(enabled, configuredLevel);
            } catch (Throwable ignored) {
            }
            return;
        }
    ...
    private void refreshQuantifiedLogging() {
        boolean logToConsole = true;
        String configuredLevel = "INFO";
        try {
            org.admany.quantified.core.common.config.MultithreadingConfig.Config config =
                org.admany.quantified.core.common.config.MultithreadingConfig.CONFIG;
            if (config != null) {
                logToConsole = config.logToConsole;
                if (config.logLevel != null && !config.logLevel.isBlank()) {
                    configuredLevel = config.logLevel.trim().toUpperCase(Locale.ROOT);
                }
            }
        } catch (Throwable ignored) {
        }

        applyQuantifiedLogging(logToConsole, configuredLevel);
    }

    private void applyQuantifiedLogging(boolean logToConsole, String configuredLevel) {
        if (configuredLevel == null || configuredLevel.isBlank()) {
            configuredLevel = "INFO";
        } else {
            configuredLevel = configuredLevel.trim().toUpperCase(Locale.ROOT);
        }

        java.util.logging.Level julLevel = quantifiedJulLevel(configuredLevel);
        Handler bridge = quantifiedJulBridge;
        if (bridge != null) {
            bridge.setLevel(logToConsole ? julLevel : java.util.logging.Level.OFF);
        }
        configureQuantifiedJul(logToConsole ? julLevel : java.util.logging.Level.OFF);

        setLog4jPackageLevel("org.admany.quantified",
            logToConsole ? configuredLog4jLevel(configuredLevel) : "OFF");
    }

    private static void configureQuantifiedJul(java.util.logging.Level level) {
        try {
            java.util.logging.Logger namespace = java.util.logging.Logger.getLogger("org.admany.quantified");
            namespace.setLevel(level);
            java.util.Enumeration<String> names = java.util.logging.LogManager.getLogManager().getLoggerNames();
            while (names.hasMoreElements()) {
                String name = names.nextElement();
                if (name == null || (!name.equals("org.admany.quantified")
                    && !name.startsWith("org.admany.quantified."))) {
                    continue;
                }
    ...
    private static java.util.logging.Level quantifiedJulLevel(String configuredLevel) {
    ...
    private static String configuredLog4jLevel(String configuredLevel) {
        return switch (configuredLevel) {
            case "TRACE" -> "TRACE";
            case "DEBUG" -> "DEBUG";
    ...
    private static void setLog4jPackageLevel(String loggerName, String levelName) {
        try {
    ...
    private static int exportChunkDebug(ServerPlayer player, String label) {
        ...
        try {
            ...
            ChatMessenger.success(player.createCommandSourceStack(),
                net.minecraft.network.chat.Component.translatable("lc2h.command.chunkdebug.export_written",
