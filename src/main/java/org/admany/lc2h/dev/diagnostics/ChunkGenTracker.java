...
public final class ChunkGenTracker {
    ...
    public static Component buildReportComponent(ChunkCoord coord) {
        if (coord == null) {
            return Component.translatable("lc2h.command.chunkinfo.invalid_coord");
        }
        ChunkGenSnapshot snapshot = snapshot(coord);
        if (snapshot == null) {
            return Component.translatable("lc2h.command.chunkinfo.no_data", coord);
        }

        MutableComponent out = Component.translatable("lc2h.command.chunkinfo.title", coord);
        out.append(Component.literal("\n"))
            .append(Component.translatable("lc2h.command.chunkinfo.counts",
        ...
        if (snapshot.lastBuildingDetail != null) {
            out.append(Component.literal("\n"))
                .append(Component.translatable("lc2h.command.chunkinfo.last_building", snapshot.lastBuildingDetail));
        }

        if (snapshot.events.isEmpty()) {
            out.append(Component.literal("\n"))
                .append(Component.translatable("lc2h.command.chunkinfo.events_none"));
            return out;
        }

        out.append(Component.literal("\n"))
            .append(Component.translatable("lc2h.command.chunkinfo.events_header", snapshot.events.size()));

        long now = System.currentTimeMillis();
        for (ChunkGenEvent event : snapshot.events) {
            long ageSec = Math.max(0L, (now - event.timeMs()) / 1000L);
            out.append(Component.literal("\n"))
                .append(Component.translatable("lc2h.command.chunkinfo.event_line",
                    TIME_FORMAT.format(Instant.ofEpochMilli(event.timeMs())),
                    ageSec,
                    event.event()));
            if (event.thread() != null && !event.thread().isBlank()) {
                out.append(Component.literal(" "))
                    .append(Component.translatable("lc2h.command.chunkinfo.thread", event.thread()));
            }
            if (event.detail() != null && !event.detail().isBlank()) {
                out.append(Component.literal(" "))
                    .append(Component.literal(event.detail()));
            }
        }
        return out;
    }
