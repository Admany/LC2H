...
public final class AsyncTerrainFeaturePlanner {
    ...
    private static final long COMPUTATION_CACHE_TTL_MS = Math.max(30_000L,
        Long.getLong("lc2h.terrainFeature.cacheTtlMs", TimeUnit.MINUTES.toMillis(20)));
    private static final int COMPUTATION_CACHE_PRUNE_SCAN = Math.max(128,
        Integer.getInteger("lc2h.terrainFeature.cachePruneScan", 512));
    private static final int COMPUTATION_CACHE_PRUNE_EVERY = Math.max(64,
        Integer.getInteger("lc2h.terrainFeature.cachePruneEvery", 128));
